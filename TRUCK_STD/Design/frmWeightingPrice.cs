...
{
    public partial class frmWeightingPrice : Form
    {
        public frmWeightingPrice()
        {
            InitializeComponent();
            // กำหนดขนาดของโปรแกรม
            this.Width = 1329;
        ...
        private void frmWeighting_Load(object sender, EventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                Log.Error("frmWeighting_load " + ex.Message);
            }
        }
        ...
        {
            gbCustomer.Location = new Point(1338, 66);
            gbProduct.Location = new Point(1338, 544);
        }
        ...
        private void saWGH_DataReceived(object sender, System.IO.Ports.SerialDataReceivedEventArgs e)
        {
            try
            {
                ...
                if (wg_in != "")
                {
                    BeginInvoke(new MethodInvoker(delegate ()
                    {
                        lbWghOut.Text = label4.Text;

                        // คำนวนหาน้ำหนักสุทธิ์
                        double a = double.Parse(lbWghIn.Text);    // น้ำหนักรอบแรก
                        double b = double.Parse(lbWghOut.Text);   // น้ำหนักรอบสอง
                        double c = 0;  // น้ำหนักสุทธิ์
                        ...
                        lbGross.Text = Convert.ToString(c);

                        // คำนวนหา ราคาสุทธิ์
                        double f = double.Parse(lbPrice.Text); // ราคา
                        double g = f * c;  // ราคาสิทธิ์

                        lbPriceNet.Text = Convert.ToString(g.ToString("#,###,###,####.00"));
                    }));
                }
            ...
            catch (IOException ex)
            {
                ...
                Log.Error("frmWeightingPrice " + ex.Message);
            }
        }
        #endregion
        ...
        private void dgvProduct_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgvProduct.Columns[e.ColumnIndex].Name == "Column8")
            {
                ...
                string prd_price = Convert.ToString(dgvProduct.Rows[e.RowIndex].Cells[3].Value);

                // กำหนดค่าที่ได้จากตัวแปรให้กับ textbox
                txtPrdCode.Text = prd_code;
                txtPrdName.Text = prd_name;
                txtPrice.Text = prd_price;
                // ปิด Groupbox 
                gbCustomer.Location = new Point(1338, 66);
                gbProduct.Location = new Point(1338, 544);
            }
            ...
            try
            {
                ...
                switch (btn.Name)
                {
                    ...
                    case "btnSaveWeightIN":
                        ...
                        // 2.2.5 บันทึกข้อมูล
                        if (tbID_FI.INSERT_ALL_DATA(txtCarRegistration.Text, label4.Text, date_in, time_in, txtPrice.Text, txtCusID.Text, txtCusName.Text, txtPrdCode.Text, txtPrdName.Text))
                        {
                    ...
                    case "btnSaveWeightOUT":
                        ...
                        if (tbWGH.INSERT_ALL_DATA(ORDER_NUMB, date_in, dateOut, time_in, timeOut, lbCarNumber.Text, wg_in, label4.Text, txtPrd_Name.Text, txtCus_Name.Text, txtPrd_Code.Text, txtCus_ID.Text, Convert.ToDecimal(lbPrice.Text), Convert.ToString(gross), Convert.ToDecimal(lbPriceNet.Text)))
                        {
                            // 2.3.6 ทำการลบข้อมูลเก่าที่อยู่ในตาราง ID_FI
                            tbID_FI.DELETE_ALL_DATA(lbCarNumber.Text);
                            // ดึงข้อมูลมาแสดงที่ dgv ใหม่
                            SHOW_DGV_WEIGH_IN();
                            // แจ้งให้ผู้ใช้ทราบ 
                            bunifuSnackbar1.Show(this, "บันทึกรายการสำเร็จ", BunifuSnackbar.MessageTypes.Success, 3000, "OK", BunifuSnackbar.Positions.TopCenter);
                        }
                        ...
                        if (registy.function.LINEState == "TRUE")
                        {
                            if (SYSTEM_LINE == "TRUE")
                            {
                                ...
                                Func_Linenotify.parameters.Add("[@Para_PricePerKG]", lbPrice.Text);
                                Func_Linenotify.parameters.Add("[@Para_PriceNet]", lbPriceNet.Text);
        ...
        private void dgvWeighIN_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (dgvWeighIN.Columns[e.ColumnIndex].Name == "Column12")
                {
                    ...
                    {
                        string carnum_h = Convert.ToString(dgvWeighIN.Rows[e.RowIndex].Cells["Column1"].Value);
                        if (tbID_FI.DELETE_ALL_DATA(carnum_h))
                        {
                            SHOW_DGV_WEIGH_IN();
                        }
                    }
                }
                else if (dgvWeighIN.Columns[e.ColumnIndex].Name == "Column3")
                {
                    string carnum_h = Convert.ToString(dgvWeighIN.Rows[e.RowIndex].Cells["Column1"].Value);
                    wg_in = Convert.ToString(dgvWeighIN.Rows[e.RowIndex].Cells["Column2"].Value);
                    string comdesc = Convert.ToString(dgvWeighIN.Rows[e.RowIndex].Cells["Column5"].Value);
                    string typedesc = Convert.ToString(dgvWeighIN.Rows[e.RowIndex].Cells["Column6"].Value);
                    ...
                    string price = Convert.ToString(dgvWeighIN.Rows[e.RowIndex].Cells["Column11"].Value);
                    string comcode = Convert.ToString(dgvWeighIN.Rows[e.RowIndex].Cells["Column7"].Value);
                    string typecode = Convert.ToString(dgvWeighIN.Rows[e.RowIndex].Cells["Column9"].Value);
